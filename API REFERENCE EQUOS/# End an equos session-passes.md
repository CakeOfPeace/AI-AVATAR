...
````yaml https://api.equos.ai/docs/v1-json patch /v1/session-passes/{id}/stop
openapi: 3.0.0
info:
  title: Equos.ai API
...
tags: []
paths:
  /v1/session-passes/{id}/stop:
    patch:
      tags:
        - SessionPasses
      summary: >-
        End an equos session. Returns null if already stopped or session was not
        found.
      operationId: SessionPassesController_stop_v1
      parameters:
        - name: id
          required: true
          in: path
